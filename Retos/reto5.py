...
def calcular_distancia(lat, long):
    ...
    for v in range(4):
        ...
        if contador3>=4:
                ...
                minimo=min(dic)
                maximo=max(dic)
                distancia1.append(minimo)
                distancia2.append(maximo)
                ...
                if kil==1:
                    bus=(minimo/16.7)
                    auto=(minimo/20.83)
                    tiempo_pro1.append(bus)
                    tiempo_pro2.append(auto)
                ...
                if kil<1 or kil>2:
                    ...
                    exit()
############################################################### Declaracion de Funciones Opcion 4 #########################################################################
...
############################################################### 4.Opcion Guardar archivo con Ubicacion Cercana ############################################################
    if opc==4:
        ...
        if cant1==0:
            ...
            exit()
        if cant1>0:
            red1 = [ round(p, 2) for p in distancia1 ]
            red2 = [ round(p, 2) for p in distancia2 ]
            red3 = [ round(p, 2) for p in tiempo_pro1 ]
            red4 = [ round(p, 2) for p in tiempo_pro2 ]
            base_de_datos = [
                {
                    "actual":(("latitud=",coord1,"longitud=",coord2)),
                    "zonawifi":(("latitud=",coord1,"longitud=",coord2),("usuarios=",usuario1,usuario2)),
                    "recorrido":(("recorrido=",red1,red2),("mediotransporte= bus y auto" ,"tiempopromedio",red3,red4)),
                }
            ]
            print(base_de_datos)
            go=int(input("¿Está de acuerdo con la información a exportar? Presione 1 para confirmar, 0 para regresar al menú principal"))
            if go==1:
...
############################################################### 5.Opcion Actualizar registros de zonas wifi desde archivo #################################################
    if opc==5:
        ho=int(input("Datos de coordenadas para zonas wifi actualizados, presione 0 para regresar al menú principal"))
